...
from .models import ModelData, CompanyData, CompanyPredict
...
class expenses_model:
  ...
  def predict_dict(self, company_data: dict) -> dict:
    ...
    BuildingExpenses = (self.building_cost + self.needs_full_cost*sum(OtherNeedsCoeffs)) * company_data['building_area']
    PatentExpenses = self.PatentPrices[company_data['patent_type']]
    BookKeepingExpenses = self.pred_duration * self._bookkeeping_formula(company_data['workers_quantity'], company_data['operations'], company_data['tax_system']) if company_data['bookkeeping'] else 0                 
    ...
    IncomeTax = Salaries*self.income_coeff if company_data['tax_system']=='ОСН' or company_data['tax_system']=='ЕСНХ' else 0
